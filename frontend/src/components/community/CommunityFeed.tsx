            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0 space-y-3">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-sm font-black uppercase tracking-tight bg-muted px-2 py-0.5 border-2 border-foreground">@{post.author}</span>
                  <Badge variant={post.verdict as 'real' | 'misleading' | 'unverified'} className="border-2">
                    {t(`verdict.${post.verdict}`).toUpperCase()}
                  </Badge>
                </div>
                <p className="text-base font-bold leading-tight border-l-4 border-foreground/20 pl-4">{post.content}</p>
                <div className="flex items-center gap-4">
                  ...
                  <Button variant="outline" size="sm" className="h-10 px-4 gap-2 font-black border-3">
