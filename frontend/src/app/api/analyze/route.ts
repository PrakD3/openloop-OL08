...
export async function POST(req: NextRequest) {
  ...
  // ── Demo mode ──────────────────────────────────────────────────────────────
  if (config.isDemo) {
    console.log(`[${ts()}] [PROXY] Running in DEMO mode — skipping real backend`);
    ...
    console.log(`[${ts()}] [PROXY] DEMO: Found precomputed result — returning after 2s delay`);
  ...
  try {
    ...
    for (let i = 0; i < MAX_POLL_RETRIES; i++) {
      ...
      if ((i + 1) % 10 === 0) {
        ...
        const secsRemaining = ((MAX_POLL_RETRIES - i - 1) * POLL_INTERVAL_MS) / 1000;
        const progressPct =
          typeof jobProgress === 'number' ? `${Math.round((jobProgress as number) * 100)}%` : '?%';
        console.log(
